...
export default function NetworkGlobe({ currentSatId = "SAT-0123" }) {
  ...
  const trackedSatellite = useMemo(() => {
    return satellites.find((sat) => sat.id === currentSatId) ?? satellites[0];
  }, [satellites, currentSatId]);
  ...
  const points = useMemo(() => [
    ...
    ...routerPoints,
    ...satellites
  ], [gatewayPoints, routerPoints, satellites]);

  const arcs = useMemo(() => {
    const activeSat = trackedSatellite;
  ...
  const rings = useMemo(() => {
    ...
    return [
      { lat: trackedSatellite.lat, lng: trackedSatellite.lng, maxRadius: 2.2, color: "#22c55e" },
      { lat: gateways[0].lat, lng: gateways[0].lng, maxRadius: 2.8, color: "#38bdf8" },
    ];
  }, [trackedSatellite]);

  return (
    <div className="relative h-full w-full min-h-125 flex justify-center items-center overflow-hidden bg-slate-950">
      <Globe
        ref={globeRef}
        width={500}
        height={500}
      ...
      <div className="absolute top-4 left-4 p-4 rounded-xl border border-cyan-500/30 bg-slate-900/80 text-[10px] text-white backdrop-blur-md pointer-events-none shadow-[0_0_30px_rgba(14,165,233,0.15)]">
      ...
      <div className="absolute bottom-4 right-4 rounded-xl border border-emerald-500/25 bg-slate-950/75 px-4 py-3 text-[10px] text-slate-100 backdrop-blur-md shadow-[0_0_24px_rgba(34,197,94,0.12)] pointer-events-none">
        <div className="mb-1 uppercase tracking-[0.35em] text-emerald-300">Tracking</div>
        <div className="font-mono text-sm font-semibold text-white">{currentSatId}</div>
        <div className="mt-1 text-slate-300">Orbit pulse and communication beam follow the tracked satellite.</div>
      </div>
    </div>
  );
}